...
- **GitHub AI bot (gateway + CPA dual-channel)**: new `github_bot/` (dual-provider `llm_client` with `xhigh→high` CPA mapping, verbatim thinking/signature passthrough, `response_id` session echo, `_redact_secrets`/`sanitize_public_error_text`; `agent_orchestrator` triage/review/explain/scan/fix-plan pipelines; `repo_scan` deterministic pre-scan; `github_runner` sticky-marker publish + allowlist labels + fingerprint-dedupe auto-issue + opt-in draft PRs) with role-separated primaries (triage `gemini-3.8-flash-high`, safety `grok-4.6`, api-compat `claude-opus-4-6-thinking`, docs `muse-spark-1.3-contributor-free`, explainer `big-pickle`, scanner `grok-composer-2.5-fast`) and cross-channel fallbacks; new workflows `ai-review.yml` / `ai-scan.yml` (weekly cron) / `ai-fix.yml` (`/fix` plans; draft PRs only with opt-in, human review required), each starting its own no-auth in-job gateway (`API_KEY` empty, `OPENCODE_PROXY_MANAGE_BACKEND=true`, anonymous out-of-box quota; external `GATEWAY_BASE_URL` secret overrides) with CPA as optional fallback; `tests/test_ai_bot.py` (34 tests); `docs/AI_BOT.md`. 新增 GitHub AI Bot（自託管 gateway 免費通道 + CPA 通道，定時掃描自動開 issue，`/fix` 規劃；opt-in 才開 draft PR，需人工審查）。
- **N×N protocol translator registry + route wiring**: new pure `src/converters/` matrix (registry/pipeline/formats/translator-types + 12 request + 12 response directed pairs for chat, responses, messages, interactions; stream + non-stream; `fidelity.ts` ledger with 3 full forward edges out of chat and 9 text-core legs; TokenCount intentionally unregistered so counting stays in the collector). Wiring: registry initializes once in `createApp` and is exposed via `ctx.translators`; `src/converters/wire.ts` Safe wrappers enforce error-envelope bypass (wrapped `{error}`, `response.failed`, `error` events, bare `{message,type}` bodies, raw SSE error strings) and per-stream holder reuse; `POST /v1/messages` inbound conversion now goes through the registry (thin wrapper over the same `anthropic.ts` pure layer, adaptive thinking collapsed to the legacy fallback for strict parity). Behavior is otherwise unchanged: wire/SSE/`[DONE]`/error-exit shapes stay route-owned, translated tools still flow through the `external__*` bridge + allowlist, translated usage is client-facing only. New `src/converters/holder.ts` + `wire.ts`, `tests/translator-wire.test.js` (fail-fast, bypass, holder, parity) and `tests/translator-integration.test.js` (live route outputs translate cross-protocol). 新增 N×N 纯函数转换矩阵与 messages 入站接线，线形与工具政策保持不变。
...
- **Anonymous first-party backend posture + Zen free-tier gate handling**: spawned backend pins `OPENCODE_CLIENT` to a known first-party value (`cli` default; `desktop`/`acp`/`app` pass through), strips the `OPENCODE_API_KEY="public"` footgun (anonymous uses empty), and git-inits the jail workspace best-effort (5s timeout, warn-and-continue) as defense-in-depth for session-dir sensitivity. Prompt `tools` maps have `false` entries stripped (true-only sent; all-false omitted) only for likely-free `opencode` models (`-free` suffix, `big-pickle`, `union-alpha`) to dodge the `FreeTierError` 403 (any `false` gates); stripped tools fall back to agent defaults server-side while paid/other providers keep the exact hard-disable map. The gate (`...used from within OpenCode` / `FreeTierError`) now surfaces as 403 `permission_denied` fail-fast (no retry, no proxy fallback) with false-positive guards for plain quota text. Jail stdout no longer prints absolute paths; interactions route gains text-level tool-markup stripping parity with chat/responses/messages. New `tests/backend-identity.test.js` / `tests/first-party-gate.test.js` / `tests/prompt-tool-overrides.test.js` / `tests/session-ids.test.js`. No new proxy knob (no config matrix change).
- **Translator parity fixes (N×N matrix review)**: chat array `content` parts no longer silently dropped (extract `text`/`content`); `responses` `incomplete` with `content_filter` reason round-trips instead of collapsing to `length` (non-stream + stream); `is_error` now round-trips via the `ERROR: ` text prefix on chat/messages/responses tool legs (strict `true`/`"true"` check; success text literally starting with `ERROR: ` round-trips as `is_error` by design); named `custom` tool declarations are kept so `custom_tool_call` invocations always have a matching declaration; ID generation centralized on `makeId(prefix)` (`toolu_` fallback separator canonicalized, prefix-only checks unaffected). N×N 翻译矩阵对齐修复。New `tests/translator-phase-fixes.test.js` (13 cases).
