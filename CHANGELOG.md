...
- **GitHub AI bot (gateway + CPA dual-channel)**: new `github_bot/` (dual-provider `llm_client` with `xhigh→high` CPA mapping, verbatim thinking/signature passthrough, `response_id` session echo, `_redact_secrets`/`sanitize_public_error_text`; `agent_orchestrator` triage/review/explain/scan/fix-plan pipelines; `repo_scan` deterministic pre-scan; `github_runner` sticky-marker publish + allowlist labels + fingerprint-dedupe auto-issue + opt-in draft PRs) with role-separated primaries (triage `gemini-3.8-flash-high`, safety `grok-4.6`, api-compat `claude-opus-4-6-thinking`, docs `muse-spark-1.3-contributor-free`, explainer `big-pickle`, scanner `grok-composer-2.5-fast`) and cross-channel fallbacks; new workflows `ai-review.yml` / `ai-scan.yml` (weekly cron) / `ai-fix.yml` (`/fix` plans; draft PRs only with opt-in, human review required); `tests/test_ai_bot.py` (25 tests); `docs/AI_BOT.md`. 新增 GitHub AI Bot（自託管 gateway 免費通道 + CPA 通道，定時掃描自動開 issue，`/fix` 規劃；opt-in 才開 draft PR，需人工審查）。
