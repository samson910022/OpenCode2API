...
Turn a local [OpenCode](https://opencode.ai) runtime into an OpenAI- and Anthropic-compatible API gateway. Use free models (GLM, Kimi, MiniMax, Muse Spark, Nemotron, Qwen, and more) from any OpenAI or Anthropic client — the free lineup rotates, so query `/v1/models` for the live list.
